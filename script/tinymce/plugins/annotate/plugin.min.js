tinymce.PluginManager.add('annotate', function (editor, url) {
    ...
    function showTooltip(e, elm) {
        var tooltipText = editor.dom.getAttrib(elm, 'data-annotation-value'); var xPos = e.clientX + Math.max(Math.round($('#myTextarea_ifr').offset().left) - ($('#editPaperWindow').width() / 2), 0); var yPos = e.clientY + Math.round($('#myTextarea_ifr').offset().top) - $('#editPaperWindow').height() - 30; if ($('#editPaperWindow').offset().left + $('#editPaperWindow').width() > $(window).width()) { xPos = $(window).width() - $('#editPaperWindow').width() - 30 }
